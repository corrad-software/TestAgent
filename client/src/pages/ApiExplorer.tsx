...
const ENDPOINTS: { group: string; endpoints: Endpoint[] }[] = [
  ...
  },
  {
    ...
    endpoints: [
      { method: "GET", path: "/users", description: "List all users with project assignments", auth: true, admin: true,
        response: 'SystemUser[] { id, email, name, role, createdAt, updatedAt, projects[] }' },
      { method: "POST", path: "/users", description: "Create a new user account", auth: true, admin: true,
        body: {
          ...
          role: { type: "string", description: "Role: Admin or Tester (default: Tester)" },
        },
        response: 'SystemUser { id, email, name, role, createdAt, updatedAt }' },
      { method: "PUT", path: "/users/:id", description: "Update user details", auth: true, admin: true,
        body: {
        ...
        response: 'SystemUser' },
      { method: "DELETE", path: "/users/:id", description: "Delete a user (cannot delete self)", auth: true, admin: true, response: '{ ok: true }' },
      { method: "PUT", path: "/users/:id/password", description: "Reset a user's password", auth: true, admin: true,
        body: { password: { type: "string", required: true, description: "New password (min 6 chars)" } },
        response: '{ ok: true }' },
      { method: "POST", path: "/users/:id/assign", description: "Assign user to a project (creates member)", auth: true, admin: true,
        body: {
          projectId: { type: "string", required: true, description: "Target project ID" },
          role: { type: "string", description: "Project role: Admin, Tester, Viewer (default: Tester)" },
        },
        response: 'Member { id, projectId, name, email, role }' },
      { method: "DELETE", path: "/users/:id/assign/:projectId", description: "Remove user from a project", auth: true, admin: true, response: '{ ok: true }' },
    ],
  },
  {
    group: "AI & Usage",
    endpoints: [
      { method: "GET", path: "/ai/usage", description: "Get AI token usage and cost summary", auth: true, admin: false,
        response: '{ totalInputTokens, totalOutputTokens, totalCost, callCount }' },
      { method: "POST", path: "/library/scenarios/:id/enrich", description: "AI-enrich a scenario's custom spec with assertions", auth: true, admin: false, sse: true,
        response: 'SSE events with enriched spec code' },
      { method: "POST", path: "/library/scenarios/:id/explain-failure", description: "AI explains why a test failed", auth: true, admin: false,
        body: { runId: { type: "string", required: true, description: "Run record ID to analyze" } },
  ...
  },
  {
    group: "App Settings",
    endpoints: [
      { method: "GET", path: "/app-settings", description: "Get application settings (API key masked)", auth: true, admin: true,
        response: 'AppSettings { model, maxIterations, defaultTestType, defaultHeadless, defaultTimeout, defaultBaseUrl, aiBudget, reportRetentionDays, ... }' },
      { method: "PUT", path: "/app-settings", description: "Update application settings", auth: true, admin: true,
        body: {
          model: { type: "string", description: "AI model (claude-haiku-4-5-20251001, claude-sonnet-4-6, claude-opus-4-6)" },
          anthropicApiKey: { type: "string", description: "Anthropic API key" },
          maxIterations: { type: "number", description: "Max AI agent iterations (1-50)" },
          defaultTestType: { type: "string", description: "Default test type" },
          defaultHeadless: { type: "boolean", description: "Default headless mode" },
          defaultTimeout: { type: "number", description: "Per-action timeout in ms" },
          defaultBaseUrl: { type: "string", description: "Default target URL" },
          aiBudget: { type: "number", description: "AI spending limit in USD (0 = unlimited)" },
          reportRetentionDays: { type: "number", description: "Days to keep reports" },
          screenshotOnFailOnly: { type: "boolean", description: "Only screenshot on failures" },
          webhookUrl: { type: "string", description: "Webhook URL for notifications" },
          webhookOnPass: { type: "boolean", description: "Notify on test pass" },
          webhookOnFail: { type: "boolean", description: "Notify on test fail" },
        },
  ...
  },
  {
    group: "Reports",
    endpoints: [
      { method: "GET", path: "/reports", description: "List all Playwright HTML reports", auth: true, admin: false,
        response: '{ runId, createdAt }[]' },
      { method: "GET", path: "/reports/:id", description: "Get report metadata", auth: true, admin: false,
        response: '{ runId, createdAt, files[] }' },
      { method: "GET", path: "/reports/:id/json", description: "Get raw JSON test results", auth: true, admin: false,
        response: 'Playwright JSON report' },
      { method: "GET", path: "/playwright-report/:runId/*", description: "Serve Playwright HTML report assets", auth: true, admin: false,
  ...
  },
  {
    group: "Daily Stats",
    endpoints: [
      { method: "GET", path: "/library/projects/:id/daily-stats", description: "Get daily run stats for 30 days", auth: true, admin: false,
        response: '{ days: { date, totalRuns, passCount, failCount, passRate }[] }' },
    ],
  },
];
